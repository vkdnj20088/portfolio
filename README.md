...
- **다중 인스턴스는 코드에만 있었습니다.** 리스와 분산 락은 워커 여럿을 전제로 짰는데 배포가 단일 인스턴스라 켜 본 적이 없었습니다. [`k8s/`](k8s/) 에서 kind 클러스터에 워커 파드 둘을 띄워 처음 켰습니다 - 작업 200건에 **겹쳐 기록된 시도 0건**, 파드 둘이 각각 일한 것을 로그로 확인합니다. 리스 잠금을 층별로 걷어낸 대조도 함께 두는데, 여기서 나온 결론은 통념과 반대입니다: **`SKIP LOCKED` 를 빼도 중복은 나지 않고** 낙관적 락이 대신 잡습니다(경합 10~44회). 그리고 시도 중 파드를 강제 종료하면 작업 **8건이 `RUNNING` 으로 남아 되살아나지 않습니다** - 회수 경로가 없다는 것을 실제 프로세스 종료로 확인했습니다. 라이브 배포는 그대로 EC2 + systemd 입니다.
