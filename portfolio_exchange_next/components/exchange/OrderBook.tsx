...
export default function OrderBook({ market, initial }: { market: string; initial: MarketSnapshot }) {
  ...
  // slot = 이 행이 놓이는 자리(0..19). key 를 가격으로 잡으면 시세가 움직일 때마다 스무 개 키가
  // 통째로 바뀌어 React 가 행 DOM 을 버리고 새로 만든다 - 화면은 같아 보여도 브라우저는 같은
  // 노드가 다른 자리로 옮겨간 것으로 보고 레이아웃 이동으로 센다(CLS 0.0136 실측).
  // 호가창은 깊이가 고정된 격자라 "몇 번째 줄"이 안정된 정체성이다.
  const row = (l: Level, side: "ask" | "bid", cum: number, slot: number) => (
    ...
    <button type="button" className={`ob-row ${side}`} key={`${side}-${slot}`}
      onClick={() => setPrice(String(l.price))}>
      <span className="ob-bar" style={{ transform: `scaleX(${max ? cum / max : 0})` }} aria-hidden="true" />
  ...
  return (
    <section className="orderbook" aria-label="호가창">
      ...
      <div className="ob-scroll" ref={scrollRef}>
        {[...asks].reverse().map((l, i) => row(l, "ask", askCum[asks.length - 1 - i], i))}
        <div className="ob-center num" ref={centerRef}>{formatKRW(snap.price)} <em>현재가</em></div>
        {bids.map((l, i) => row(l, "bid", bidCum[i], i))}
      </div>
    </section>
  );
}
