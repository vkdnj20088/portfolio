...
const GENERAL_REPLIES = [
  // 마크다운 서브셋(#C1)을 쓰는 문안. 목록/강조/인라인코드가 실제로 렌더되는지 화면에서 보이게 한다.
  '요약하면 세 가지입니다.\n\n- 상태의 **출처를 하나로** 유지하세요\n- 파생 가능한 값은 저장하지 말고 계산하세요\n- 비동기 흐름에는 반드시 *실패 경로*를 함께 설계하세요\n\n마지막 항목이 가장 자주 빠집니다.',
  ...
  '시간대 버그는 저장과 표시를 섞을 때 생깁니다. 저장은 항상 `UTC` 한 기준으로, 변환은 표시 직전에 한 번만 하세요. 중간 계층이 시간을 만지기 시작하면 끝이 없습니다.',
  '데이터 모델은 화면보다 오래 삽니다. 화면에 맞춰 모델을 구부리지 말고, 사실을 그대로 담은 모델에서 화면용 형태를 파생하세요.',
  '모듈 경계는 감추는 것이 반입니다. 내보내는 것을 최소로 유지하면 내부를 바꿀 자유가 그만큼 남습니다 - [공개한 것은 전부 약속](#)이 됩니다.',
