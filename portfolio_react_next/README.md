...
### 이중 승인 실험대 - timeout 은 실패가 아니다

`/approval` 은 이 앱의 다섯 번째 화면입니다. 앞의 네 화면이 "무엇을 답하는가"와 "무엇을 막는가"
였다면, 여기는 **외부가 대답하지 않을 때 무엇을 적을 것인가**입니다.

**왜 새 데모인가.** `portfolio_jquery_spring` 의 작업 릴레이는 재시도의 결정성과 관측
(타임라인·DLQ·아웃박스 대조)을 증명합니다. 다만 그 상태값에는 '모름'이 없어 timeout 을 실패로
보고 재시도합니다 - 부작용이 없는 시뮬레이션이라 그것이 안전했습니다. 이 데모는 그 빈칸,
**부작용을 되돌릴 수 없어 재시도 자체가 위험한 경우**를 다룹니다. 겹치지 않고 이어집니다.

증명하려는 것은 한 문장입니다. 외부 결제 승인이 timeout 일 때 그것은 실패가 아니라 **모름**이고,
성공/실패 2 값으로 접으면 **어느 쪽으로 접어도 틀립니다**. 재시도하면 같은 결제가 두 번 청구되고,
포기하면 승인될 수 있었던 결제를 잃습니다. 해법은 세 번째 상태와 **승인 조회 대사**입니다.
PG 도메인을 고른 이유는 업계에 이미 "승인 조회"와 "망취소"라는 표준 대응이 있어서, 제가 지어낸
개념이 아니라는 것이 도메인 자체로 증명되기 때문입니다.

**끄는 경로를 새로 만든 것이 이 화면의 본체입니다.** 방어를 켠 화면만 보여 주면 "원래 안 일어날
일 아니냐"는 의심을 지울 수 없습니다. 그래서 방어선 넷을 각각 끌 수 있게 만들고, 같은 시나리오를
끈 쪽과 켠 쪽으로 나란히 돌립니다.

| 방어선 | 끄면 벌어지는 일 |
|---|---|
| 진입점 멱등키 | 같은 키의 재제출이 결제 요청을 하나 더 만듭니다 |
| 조건부 전이 클레임 | 기대 상태를 보지 않고 덮어써서 두 처리자가 모두 승인 요청을 보냅니다 |
| 승인 조회 대사 | timeout 을 조회 없이 2 값으로 접습니다(재시도 / 포기 중 선택) |
| 시도 상한 | 실패의 끝이 없어 같은 요청을 계속 다시 보냅니다 |

여기에 **회수 목적지**가 하나 더 있습니다. 처리 중 죽은 워커의 클레임을 회수할 때 '접수'로
되돌릴지 '모름'으로 보낼지인데, 이것은 방어선의 유무가 아니라 **그럴듯한 오답**입니다.

프리셋 여섯 개가 각각 한 번의 클릭으로 대조를 만듭니다. 아래 숫자는 커밋된 실행 기록이고,
화면은 같은 값을 지금 이 자리에서 다시 계산합니다.

| 프리셋 | 뺀 쪽 | 켠 쪽 | 무엇을 증명하는가 |
|---|---|---|---|
| P1 응답 유실 후 재시도 | 승인 2건 | 1건 | timeout 은 실패가 아닙니다 |
| P2 승인 전 끊김 | 승인 0건 · 격리 | 1건 | 반대로 접어도 틀립니다 |
| P3 워커 두 대가 동시에 | 승인 2건 | 1건 | 방어선의 위치는 저장소입니다 |
| P4 더블클릭 | 요청 2건 · 승인 2건 | 1건 | 진입점에서 걸러야 하는 중복이 있습니다 |
| P5 연결 실패가 계속될 때 | 호출 6회 · 접수 상태 | 호출 3회 · 격리 | 실패의 끝을 정의해야 합니다 |
| P6 워커가 죽은 뒤의 회수 | 승인 2건 | 1건 | 그럴듯한 오답 |

**P2 와 P6 이 이 데모의 차별점입니다.** 나머지가 방어선의 유무를 보이는 대조라면, 이 둘은
성실하게 짠 코드가 내는 오답을 실제로 실행해 숫자로 틀렸음을 보입니다. timeout 을 실패로 보고
포기하는 것도, 죽은 워커의 작업을 처음으로 되돌리는 것도 그 자체로는 합리적으로 보입니다.

**상한을 둘로 갈라 셉니다.** 전송 시도 상한(우리가 PG 를 몇 번 두드렸는가)과 조회 실패 상한
(그 결과를 몇 번 확인하려다 실패했는가)은 다른 상한입니다. 한 칸에 세면 "보내다 실패했다"와
"보낸 결과를 모른다"가 같은 칸에 들어가 격리 사유가 뭉개집니다. 격리 사유가 실제로 갈리는지는
테스트가 문자열로 확인합니다.

**시드가 없습니다.** 이 엔진에는 난수가 하나도 없어서(`Math.random` 0건) 시드가 결정성에
기여하는 바가 0입니다. 시간과 ID 만 주입하면 실행 전체가 결정적이고, 재현의 단위는 시드가 아니라
시나리오 파라미터(PG 모드 · 워커 수 · 가드 조합)입니다. 릴레이 데모가 Mulberry32 를 쓰는 이유는
그쪽이 성패를 확률로 뽑기 때문이고, 여기는 성패가 결정된 입력입니다.

**커밋된 산출물의 역할이 다릅니다.** 다른 데모의 커밋 파일은 "배포에 키가 없어 재생한다"는
장치지만, 이 엔진은 브라우저에서 그대로 돕니다. 그래서
`pnpm --filter @chat/docqa run make:approval-runs` 가 남기는 12행은 재생용이 아니라
**표류 감지기**입니다. 커밋된 숫자와 지금 계산한 숫자가 갈리면 엔진이 바뀐 것이고, 화면과
테스트가 같은 대조를 봅니다. 산출물에 수집 시각을 넣지 않았습니다 - 실행이 결정적이라 시각만
바뀌면 내용이 같은 diff 가 매번 생기고, 그러면 이 파일이 신호가 아니라 잡음이 됩니다.

**증명하지 않는 것을 화면에 적습니다.** 진짜 병렬이 아니고(워커 두 대는 같은 프로세스 안에서
차례로 돕니다), 실제 DB 락 경합은 다루지 않았고, 크래시를 실제로 주입하지 않았고, 성능은 재지
않았습니다. 회수 임계 시간도 실측이 아닙니다. 가장 큰 것은 **승인 조회 API 가 있다는 가정**입니다 -
이 가정이 깨지면 여기 있는 해법이 통째로 성립하지 않으므로, 후퇴 경로 세 단계를 화면에 함께
적었습니다. 망취소는 범위 밖이라 만들지 않았고, 만들지 않았다고 적었습니다.

**인트로 카드는 아홉 장 그대로입니다.** 에이전트 층과 같은 이유로 열 번째 카드를 만들지 않고
목록 밖 링크 한 줄로 걸었습니다 - 카드가 되면 "선택된 작업 09"와 데모 개수 문구가 전부
흔들립니다. 상단 내비는 다섯 번째 항목으로 늘렸습니다. 에이전트 층의 세 화면은 같은 데이터를
다르게 보는 것이라 세그먼트로 묶었지만, 실험대는 주제가 달라 묶을 상위 개념이 없습니다.
...
- 단위 134개: `@chat/search-domain` 42(토크나이즈 불변식/결정성/키워드-시맨틱 대비/추출 span/근거 검증/
  **골드셋 회귀 게이트**/**LLM 대조군 채점**: 부분 수집을 침묵으로 세지 않는가, LLM 이 이긴 문항이
  표에 남는가, 산출물이 낡았는지 감지하는가) + `@chat/docqa` 92(도구 계약과 결정성, guard 도구의 `X-Request-Id` 전파, 시나리오 다섯, 커밋 산출물 형태, **종료 상태 판정**: 답한 뒤 덧붙인 단서를 거절로 읽지 않는가,
  **승격 자산 무결성**과 **요약-원본 대조**, **가드 시나리오**: 과제가 주입을 미리 일러 주지 않는가,
  악의 없이도 막히는 경우를 목록이 갖고 있는가, **실험대 상태**: 시나리오는 양쪽에 같이 적용되는가,
  손대면 기대값 표식이 떨어지는가, **커밋 산출물 표류**: 지금 계산한 숫자와 갈리지 않는가,
  SSE 파서의 바이트 경계/CRLF/손상 페이로드, 강등 시
  무중복 이어받기, **라우트 핸들러 거절/SSE 계약**, **CSP 해시 핀 정합**, 하이라이트 RTL)
  + 하네스 `@chat/agent-core` 96(McNemar 정확검정의 손계산 대조, 부트스트랩 결정성과 케이스 단위
  재표집, 검정력 경계, kappa 의 쏠림 취약성, 판정 문구 매핑, **가드레일**: 신뢰 불가 인자는
  승인으로도 풀리지 않는가, 짧은 값을 문서 출처로 오인하지 않는가, 가드를 끄면 대조군이 성립하는가)
  + 엔진 `@chat/approval-domain` 58(전이표의 역행 거부, 조건부 전이가 선점을 오류와 구분하는가,
  timeout 두 종류를 스텁이 실제로 갈라 만드는가, 전 구간 흐름 25: 정상 경로 · 응답 유실 · 중복
  전달과 워커 두 대 · 취소 경합 · 소비자 fan-out · 멈춘 클레임 회수 · 상한 도달 격리,
  **프리셋 대조**: 여섯 프리셋의 끔/켬 기대값이 실행과 맞는가, 상한 두 종류의 격리 사유가 갈리는가,
  같은 설정이 타임라인까지 같은가)
