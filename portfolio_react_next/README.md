...
- **CI 실 API 호출 0** - 캐시 계약(재요청 결정성/진행 중 합류/중단 생존/실패 비캐시/LRU)은
  fake 스트림 주입으로, 키 게이트와 무키 폴백은 환경변수 스텁으로 검증합니다(테스트 10종).
...
> 키 게이트/무키 폴백 3)가 앱 계층에 추가됐습니다. 현재 최종 수치는 **단위 149개(도메인 49 +
> 앱 59 + UI 41) + e2e 4개(Playwright 스펙 3파일, 스크롤 앵커링/응답 도착/reduced-motion -
