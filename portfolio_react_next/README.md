...
설치 없이 바로 확인하실 수 있습니다. 홈의 추천 질문 칩을 누르면 방 생성부터 응답
페이드 인까지 핵심 흐름이 한 번에 재생됩니다.

![AI Chat 홈 화면](docs/home.png)

| | |
|---|---|
| 프레임워크 | Next.js 15.5.20 (App Router) / React 19.2 |
| 언어 | TypeScript 6.0 (strict) |
| 모노레포 | pnpm workspace + Turborepo 2 |
| 스타일 | CSS Modules + CSS 커스텀 프로퍼티 토큰 |
| 지원 브라우저 | Chrome 88+ (Edge 88+ / Firefox 85+ / Safari 14+) |

## 실행 방법

```bash
# Node 18.18+ 필요. pnpm 은 corepack 으로 준비됩니다(corepack 이 없다면: npm i -g pnpm).
corepack enable
...
| 영역 | 선택 | 제출 시점 최신 | 판단 |
|---|---|---|---|
| Next.js | **15.5.20** | 16.2 | 요구사항이 15 를 지정 -> 15 계열 최신 패치 |
| React | **19.2.7** | 19.2.7 | 최신 안정을 **정확 고정**했습니다. 고정 기준은 "산출물을 결정하는가"입니다 - 번들에 실리는 런타임(next/react/react-dom)과 산출물을 만들어내는 컴파일러(typescript)는 정확 고정, 개발 도구는 캐럿+lockfile. `@chat/ui` 의 peerDependencies 는 `^19` 범위를 유지합니다(라이브러리는 호환 범위를 선언하고, 고정은 앱의 몫) |
...
| Node.js | engines `>=18.18` / `.nvmrc` **24.18 LTS** | 24 LTS | 개발/검증은 24.18.0 에서 수행했습니다(`@types/node` 24 동행). engines 하한은 Next 15 의 하한과 일치시켜 평가 환경을 넓게 포용합니다 - `engine-strict` 가 켜져 있어 하한을 올리면 구버전 Node 평가 환경에서 설치가 실패합니다 |
| ESLint | **9 계열 (9.39)** | 10.7 | 10 은 세 겹으로 창 밖입니다: `eslint-plugin-react` 는 최신(7.37.5)조차 peer 상한이 `^9.7` 이고, Next 15.5 에 버전이 묶인 `@next/eslint-plugin-next` 는 10 지원 선언이 없으며, eslint 10 의 engines(Node 20.19+/22.13+/24+)가 `engine-strict` 하에서 평가자 설치 하한을 사실상 끌어올립니다 |
| Prettier | 3.4 | 3.9 | 작업 기간 중 포매터 버전업은 산출 포맷을 흔들 수 있어 동결 |

원칙은 하나입니다 - **버전은 취향이 아니라 호환 창의 함수**입니다. 하한(Chrome 88, Node 18.18)은
요구사항과 프레임워크가 정하고, 상한은 도구체인의 지원 범위가 정합니다. 그 창 안에서 최신을 씁니다.

### 지원 브라우저 - 기본값을 그대로 두면 요구사항을 못 맞춥니다

**Next.js 의 기본 타깃은 Chrome 111+ 입니다.** 요구 범위인 Chrome 88 과 23 버전 차이라,
`browserslist` 를 명시하지 않으면 지원 범위를 벗어난 코드가 그대로 나갑니다.

```jsonc
// apps/chat/package.json
"browserslist": ["chrome >= 88", "edge >= 88", "firefox >= 85", "safari >= 14"]
```

Chrome 88 만 적으면 다른 브라우저에는 트랜스파일 기준이 서지 않습니다. 같은 시기(2020 말~2021 초)에
릴리스된 버전을 함께 적어 실효 하한을 맞췄습니다.
...
타깃만 바꿔도 청크 구성(7↔8개)과 콘텐츠 해시가 달라집니다 - 선언이 트랜스파일 타깃으로
실제 반영된다는 뜻입니다.

다만 이 확인만으로는 부족해서, **검증을 두 겹으로 걸었습니다.**

1. **문법**: `pnpm check:browser-support` - es-check 로 산출물 전체가 ES2021 을 넘지 않는지
   검사합니다. Chrome 88 은 ES2021 을 완전히 지원하고 ES2022 부터 갈리므로(`at()` 92,
   `Object.hasOwn` 93, class static block 94, top-level await 89) **ES2021 이 정확한
   경계선**입니다. 산출물 전 청크가 통과합니다(빌드마다 검사가 함께 돕니다).
2. **런타임 API**: 문법 검사는 `crypto.randomUUID()`(Chrome 92+) 같은 **런타임 API 하한
   위반을 잡지 못합니다** - 구문상 합법이라 es-check 와 빌드를 통과하고, Chrome 88 에서
   `TypeError` 로만 드러납니다. 트랜스파일러는 문법만 낮출 뿐 API 를 만들어 주지 않기
   때문입니다. 그래서 하한을 넘는 API 를 **ESLint 금지 규칙**으로 막았습니다:
   `crypto.randomUUID`(92+) / `structuredClone`(98+) / `Object.hasOwn`(93+) /
   `.at()/findLast`(92+/97+) / `AbortSignal.timeout`(103+). 위반은 린트 단계에서 실패합니다.
...
- **사이드바**: 각 페이지가 아니라 **루트 레이아웃이 소유**합니다. 페이지마다 목록을 렌더링하면
  홈↔채팅방 이동 시 목록이 언마운트/리마운트되면서 스크롤 위치와 로딩 상태가 초기화됩니다.
...
- id 는 `crypto.randomUUID`(Chrome 92+, 하한 초과 - 환경 구성 린트 게이트가 차단) 대신 자체 생성기를 씁니다.
...
- vitest 는 **3 계열**을 택했습니다 - 4 는 engines 가 Node 20/22/24+ 전용이라 저장소 하한(Node 18.18)과
  충돌합니다(환경 구성 버전 정책과 같은 기준).
...
| 요구 | 구현 |
|---|---|
| 사이드바에 채팅방 목록 | 루트 레이아웃 소유(환경 구성 설계 그대로) - 홈↔채팅방을 오가도 목록이 리마운트되지 않습니다 |
...
판정은 두 축입니다 - 현행 v5 는 브라우저 하한이 Chrome 91+ 로 요구(88) 밖이고, 지원 창
안에 있는 v4(Chrome 73+, 4.42 부터는 React 19 peer 도 선언 - npm 실측)는 유지보수
세대인 데다 다음 메이저가 91+ 를 요구해 **Chrome 88 요구가 살아 있는 한 업그레이드
경로가 없는 의존성**이 됩니다. 갇힐 라이브러리라면 직접 구현이 정직한 선택입니다.
...
- 조각마다 `animation-delay = 배치 내 순번 × 40ms` 를 주는 **CSS 스태거**입니다.
...
`danger` 는 관례(빨강)를 유지해 브랜드와 같은 색군에 공존합니다 - 삭제 버튼은 ✕
...
- **무엇이 재시도 대상인가(경계 명료화)** - 회선 끊김(`reader.read()` 거부), 불완전 스트림(`done`
  없이 종료), 5xx 는 **일시적 전송 실패**라 지수 백오프(300/2ⁿ, 상한 2s)로 재연결합니다. 반면
...
와이어프레임의 "목록 화면 ↔ 채팅 화면" 전환을 한 화면에서 얻습니다. 방을
...
- **주제 21종 × 4문안 + 일반 16문안 = 100종.** 인사/테스트/네이밍/리뷰/디버깅/
...
> **SSE 회복탄력성(자동 재연결/결정적 이어받기) 테스트 4개**와 **가변 높이 windowing 코어
> 테스트 6개**(렌더 성능 라운드)가 앱 계층에 추가됐습니다. 현재 최종 수치는 **단위 101개(도메인 46 + 앱 34 +
> UI 21) + e2e 4개(Playwright 스펙 3파일, 스크롤 앵커링/응답 도착/reduced-motion - jsdom 이
> 못 재는 것만 실브라우저로)**
> 입니다.
...
같은 자리에 둔 이유는 재사용입니다 - 디자인 토큰(`@chat/ui`), ESLint/TS 설정, Chrome 88 상한
...
- 단위 51개: `@chat/search-domain` 31(토크나이즈 불변식/결정성/키워드-시맨틱 대비/추출 span/근거 검증/
  **골드셋 회귀 게이트**) + `@chat/docqa` 20(SSE 파서의 바이트 경계·CRLF·손상 페이로드, 강등 시
  무중복 이어받기, **라우트 핸들러 거절·SSE 계약**, **CSP 해시 핀 정합**, 하이라이트 RTL)
- Lighthouse 4개 영역 **100/100/100/100** (세 라우트 × 데스크톱/모바일)
- 요청별 nonce CSP + strict-dynamic(FOUC 스크립트는 sha256 고정) + 보안 헤더 4종 - 4앱 동일 정책
- 360px 오버플로 0, 콘솔 0, ES2021 상한 검사 통과
