...
하네스 도메인은 `@chat/agent-core` 라는 별도 패키지입니다(단위 34개). 브라우저 없이 테스트가 돌고, 2단계에서 다른 앱이 같은 하네스를 쓸 수 있습니다.
...
- 단위 82개: `@chat/search-domain` 42(토크나이즈 불변식/결정성/키워드-시맨틱 대비/추출 span/근거 검증/
  **골드셋 회귀 게이트**/**LLM 대조군 채점**: 부분 수집을 침묵으로 세지 않는가, LLM 이 이긴 문항이
  표에 남는가, 산출물이 낡았는지 감지하는가) + `@chat/docqa` 40(도구 계약과 결정성, guard 도구의 `X-Request-Id` 전파, 시나리오 다섯, 커밋 산출물 형태, SSE 파서의 바이트 경계/CRLF/손상 페이로드, 강등 시
