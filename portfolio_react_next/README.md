...
- 단위 104개: `@chat/search-domain` 42(토크나이즈 불변식/결정성/키워드-시맨틱 대비/추출 span/근거 검증/
  **골드셋 회귀 게이트**/**LLM 대조군 채점**: 부분 수집을 침묵으로 세지 않는가, LLM 이 이긴 문항이
  표에 남는가, 산출물이 낡았는지 감지하는가) + `@chat/docqa` 62(도구 계약과 결정성, guard 도구의 `X-Request-Id` 전파, 시나리오 다섯, 커밋 산출물 형태, **종료 상태 판정**: 답한 뒤 덧붙인 단서를 거절로 읽지 않는가,
  **승격 자산 무결성**과 **요약-원본 대조**, SSE 파서의 바이트 경계/CRLF/손상 페이로드, 강등 시
  무중복 이어받기, **라우트 핸들러 거절/SSE 계약**, **CSP 해시 핀 정합**, 하이라이트 RTL)
  + 하네스 `@chat/agent-core` 79(McNemar 정확검정의 손계산 대조, 부트스트랩 결정성과 케이스 단위
  재표집, 검정력 경계, kappa 의 쏠림 취약성, 판정 문구 매핑)
