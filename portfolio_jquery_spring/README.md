...
두 화면은 한 애플리케이션이며, 배포에서 서브도메인으로만 나눠 두었습니다.
...
| **동시성** | 5. 커스텀 200개 경계(TOCTOU) / 12. 고정 확장자 토글(낙관적 락) / 28. IP 규칙 수정과 낙관적 락 |
...
| `FlywayMigrationMySqlIT` *(통합)* | V1~V6 적용 + `ddl-auto=validate` 통과 = 마이그레이션-엔티티 정합 |

> `./gradlew test` - 총 138개(H2, Docker 불필요) 그린. *(통합)* 4종은 Testcontainers 로
