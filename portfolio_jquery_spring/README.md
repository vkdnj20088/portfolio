...
별도 설치 없이 바로 열립니다. 로컬 구동은 아래 "실행 방법"에 있습니다.
세 화면(파일 확장자 차단, IP 접근 제어, 작업 릴레이)은 한 애플리케이션이고 배포에서 주소로만 나눠 두었습니다.
...
| 커스텀 확장자 **삭제(X)** | `DELETE /api/extensions/custom/{id}` (204) | 칩의 X 클릭 |
...
> **번들을 저장소에 커밋해 둔 이유**: `./gradlew bootRun` / `test` 가 **Node 설치 없이** 돌도록, 프론트엔드 산출물(`static/js/bundle.<hash>.js` / `static/css/style.<hash>.css` 와 두 파일명이
...
- **brotli 는 도입하지 않았습니다 - 실측 결과 이득이 없어서입니다.** Ubuntu 24.04 에 공식 모듈
  ...
  **이미 Terser 로 minify 되어 brotli 가 얻을 잉여가 거의 없었고**(479B, 1%), 의미 있는 차이가 나는
  q11(9% 증가)은 온더플라이 압축에 CPU 가 과해 t3.micro 에 부적합합니다. 사전 압축(`brotli_static`)은
...
- **회귀 경계** - 기존 keyset/Instant/검색 설계는 그대로입니다. `VARCHAR(45)` 는 IPv6+`/128`(43자)까지
  수용해 **스키마 변경이 없습니다**. 잔여 심화(CIDR 인덱스 검색, OFFSET 대 keyset 벤치)는 옵션.
...
- **왜 MySQL 로 재현하나** - 깊은 페이지에서 OFFSET 저하는 **디스크/버퍼풀 기반 DB**에서 실재합니다.
  ...
  V3)를 곧장 seek 합니다. H2 인메모리는 스킵이 거의 공짜(실측 ~60us)라 차이가 안 드러나므로, 벤치는
...
- **표시값 = 실행값** - 화면의 "+2.1s (기저 1s x 2^1, 지터 +0.1s)"는 실제로 그만큼 기다립니다.
...
> OFFSET 대 keyset 지연 벤치는 `./gradlew benchmarkTest` 로 별도 분리(Docker 필요).
